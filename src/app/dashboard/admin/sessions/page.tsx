...
export default async function AdminSessionsPage() {
  ...
  // Fetch all sessions including default sessions for admin
  const allSessions = await Session.find({})
    .sort({ isDefault: -1, createdAt: -1 }) // Default sessions first, then by creation time
    .lean()

  return (
    <div className="text-right">
      <div className="mb-6 lg:mb-8">
        <h1 className="text-2xl lg:text-3xl font-bold">إدارة الجلسات</h1>
        <p className="text-muted-foreground mt-2">إدارة جميع جلسات واتساب بما في ذلك الجلسات الافتراضية</p>
      </div>
