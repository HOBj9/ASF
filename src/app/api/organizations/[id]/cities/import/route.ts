...
export async function POST(
  ...
  try {
    ...
    const governorateId = (formData.get('governorateId') as string | null)?.trim();

    if (!governorateId) {
      return NextResponse.json({ error: 'يجب تحديد المحافظة' }, { status: 400 });
    }
    ...
    for (let i = 0; i < rows.length; i++) {
      ...
      try {
        await cityService.create(organizationId, {
          governorateId,
          name: name.trim(),
          nameAr: nameAr || null,
          order: order ?? 0,
