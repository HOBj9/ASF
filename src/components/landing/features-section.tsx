...
export function FeaturesSection() {
  ...
  const mobileLineHostRef = useRef<HTMLDivElement | null>(null)
  ...
  useEffect(() => {
    const calculateStopPoints = () => {
      const lineHost = isDesktop ? desktopLineHostRef.current : mobileLineHostRef.current
      ...
      const hostSize = isDesktop ? hostRect.width : hostRect.height
      if (hostSize <= 0) {
        setStopPoints(buildFallbackStopPoints(cardCount))
        return
      }
      ...
      const nextStops = Array.from({ length: cardCount }, (_, index) => {
        ...
        const iconCenter = isDesktop
          ? iconRect.left + iconRect.width / 2 - hostRect.left
          : iconRect.top + iconRect.height / 2 - hostRect.top
  ...
  return (
    ...
    >
      <RouteMarker stopId="features" index={2} dir={dir} />
      <div className={landingContainer}>
        ...
        {isDesktop ? (
              ...
              {cards.map((feature, index) => {
                ...
                return (
                  <div key={feature.title} className="relative min-h-[350px]">
                    ...
                    >
                      <div className={cn("h-full bg-card/95", premiumCardBase, premiumCardHover, "p-5")}>
          ...
          <div className="relative lg:hidden">
            <div ref={mobileLineHostRef} className="absolute bottom-8 left-6 top-6 w-px bg-border/70">
              <motion.div
            ...
            <div className="space-y-5">
              {cards.map((feature, index) => {
                ...
                return (
                  <div key={feature.title} className="relative min-h-[120px] ps-16">
                    <motion.div
                      ref={(el) => {
                        mobileIconRefs.current[index] = el
                      }}
                      className="absolute start-6 top-8 z-20 flex h-9 w-9 -translate-x-1/2 items-center justify-center rounded-full border border-primary/35 bg-background shadow-[0_8px_18px_-14px_hsl(var(--primary)/0.65)]"
                      initial={false}
                      animate={{
                        opacity: iconIsVisible ? 1 : 0.1,
                        scale: iconIsVisible ? 1 : 0.7,
                    ...
                    <motion.div
                      className={cn("bg-card/95 p-5", premiumCardBase, premiumCardHover, isTopPattern ? "origin-bottom" : "origin-top")}
                      initial={false}
                      animate={{
                        opacity: cardIsVisible ? 1 : 0,
